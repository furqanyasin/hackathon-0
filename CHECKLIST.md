...
> Complete everything in Bronze first.

### Additional Watchers (need 2+)
- [x] `filesystem_watcher.py` ✅ (counts as watcher 1)
- [x] `gmail_watcher.py` ✅ (counts as watcher 2 — needs credentials)
- [ ] `whatsapp_watcher.py` — Playwright-based, watches for keywords (`urgent`, `invoice`, `payment`)
- [ ] All watchers running via PM2 and surviving reboots (`pm2 save && pm2 startup`)

### LinkedIn Auto-Posting
- [ ] Script or MCP to automatically post business updates on LinkedIn
- [ ] Posts generated by Claude based on `Business_Goals.md`

### Claude Reasoning Loop
- [ ] Claude reads `/Needs_Action/`, creates `PLAN_*.md` in `/Plans/` with checkbox steps
- [ ] Plan files include `status: pending_approval` for sensitive steps
- [ ] Plans are processed and moved to `/Done/` on completion

### MCP Server (at least 1)
- [ ] Set up `email-mcp` in `~/.claude/claude_desktop_config.json` (Windows MCP config path)
- [ ] Claude can invoke MCP to send emails
- [ ] Test MCP connection

### Human-in-the-Loop (HITL)
- [ ] Claude writes approval files to `/Pending_Approval/` instead of acting directly
- [ ] Orchestrator watches `/Approved/` and triggers MCP action when files appear
- [ ] Files move to `/Done/` after action completes

### Scheduling (Windows Task Scheduler)
- [ ] Schedule daily Claude trigger via Windows Task Scheduler
  - Action: `uv run python orchestrator.py`
  - Trigger: daily at 8:00 AM

### Agent Skills
- [ ] All Silver features implemented as Agent Skills
